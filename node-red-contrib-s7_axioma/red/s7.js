...
module.exports = function (RED) {
    ...
    function S7Endpointaxioma(config) {
        ...
        function doCycle() {
            if (!readInProgress && connected) {
                ...
                if (readDeferred > 10) {
                    ...
                    connect(connOpts); //this also drops any existing connection
                }
            }
        }
        ...
        function connect(connOpt) {
            function doConnect() {
                ...
                node._conn.initiateConnection(connOpt, onConnect);
            }
...
//		connect(connOpts)
    ...
    RED.nodes.registerType("s7 endpoint axioma", S7Endpointaxioma);

    // ---------- S7 In ----------

    function S7Inaxioma(config) {
        var node = this;
        var statusVal;
        RED.nodes.createNode(this, config);
    ...
    RED.nodes.registerType("s7 in axioma", S7Inaxioma);

    // ---------- S7 Out ----------

    function S7Outaxioma(config) {
        var node = this;
        var statusVal;
        RED.nodes.createNode(this, config);
    ...
    RED.nodes.registerType("s7 out axioma", S7Outaxioma);


    // ---------- S7 Control ----------

    function S7Controlaxioma(config) {
        var node = this;
        var statusVal;
        RED.nodes.createNode(this, config);
    ...
    RED.nodes.registerType("s7 control axioma", S7Controlaxioma);
	
	function siemens_flex_connaxioma(config) {
        RED.nodes.createNode(this,config);
        var node = this;
        node.on('input', function(msg) {
			node.endpoint = RED.nodes.getNode(config.endpoint);
			 ...
			 { 
				...
				rack: msg.payload.rack,
				slot: msg.payload.slot, 
			 }


			node.endpoint.connect(msg.payload);
			
			var connectionas = node.endpoint._conn
			node.send(conas);
    ...
    RED.nodes.registerType("siemens_flex_conn axioma",siemens_flex_connaxioma);
	
	function siemens_flex_varaxioma(config) {
        RED.nodes.createNode(this,config);
        var node = this;
        node.on('input', function(msg) {
			node.endpoint = RED.nodes.getNode(config.endpoint);
			node.endpoint._vars = msg.data
    ...
    RED.nodes.registerType("siemens_flex_var axioma",siemens_flex_varaxioma);
};
